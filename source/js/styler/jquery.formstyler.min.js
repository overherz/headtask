...
m+'<ul style="position: relative; list-style: none; overflow: auto; overflow-x: hidden">'+v+"</ul></div>");d.append(e);var n=c("ul",e),f=c("li",e),t=c("input",e),w=c("div.jq-selectbox__not-found",e).hide();f.length<h.selectSearchLimit&&t.parent().hide();var x=0;f.each(function(){c(this).css({display:"inline-block","white-space":"nowrap"});c(this).width()>x&&(x=c(this).width());c(this).css({display:"block","white-space":"normal"})});d.width(a.outerWidth(true));x>e.width()&&e.width(x+e.width()-f.width());
...
p()});a.on("adaptiveWidth",function(){a.css({position:"static"});a.next().width(a.outerWidth(true));a.css({position:"absolute"})});c(window).on("resize",function(){a.trigger("adaptiveWidth")})}}):a.is(":reset")&&a.click(function(){setTimeout(function(){a.closest(h.wrapper).find("input, select").trigger("refresh")},1)})})}})(jQuery);