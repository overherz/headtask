...
//игнорировать ссылки с расширениями:
        $this->set_ignore(array("javascript:", ".css", ".js", ".ico", ".jpg", ".png", ".jpeg", ".swf", ".gif",".xml"));
...
//echo "</pre>";
        ...
        if ($map)
        {
            ...
            file_put_contents(ROOT."sitemap.xml",$map,LOCK_EX);
        }
    }
    ...
    private function multi_curl($urls)
    {
        ...
        foreach($curl_handlers as $curl)
        {
            ...
            if(curl_errno($curl) == CURLE_OK)
            {
                ...
                //parsing content
                $this->parse_content($content,curl_getinfo($curl, CURLINFO_EFFECTIVE_URL));
            }
        }
    ...
    public function get_links($domain){
        ...
        {
            $this->sitemap_urls[] = \Router::url_parse($this->domain,true);
        }
        ...
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
        $page = curl_exec($curl);
        $from = curl_getinfo($curl, CURLINFO_EFFECTIVE_URL);
        curl_close($curl);
        $this->parse_content($page,$from);
    }
    ...
    private function parse_content($page,$from)
    {
        ...
        for($i = 1; $i < $size; $i++)
        {
            ...
            foreach($match[$i] as $url)
            {
                ...
                if(strpos($url, "mailto") === false && strpos($url, "http") === false && trim($url) !== "")
                {
                   // if($url == "?page=1") pr($from);
                    ...
                    if($url[0] == "?" && strpos($from, "?") === false) $url = $from.$url;
                    else if($url[0] == "?" && strpos($from, "?") !== false) $url = \Router::url_parse($from).$url;
                    else $url = $this->protocol.$this->base."/".$url;
                }
                //if new and not empty
                $url = \Router::url_parse($url,true);
