...
class calendar_new extends \Controller {
    ...
    function default_for_this()
    {
        ...
        $current_end = "{$year}-{$month}-{$currentMonth->countDays()}";

        $w  = date("w",strtotime($current));
       // pr($w);

        foreach ($tasks as &$t)
        {
          //  $t['start'] = strtotime($t['start']);
          //  $t['end'] = strtotime($t['end']);
            //$t['start'] = explode("-",$t['start']);
           // if ($t['start'] < $current) $t['start'] = date("Y-m-d",strtotime($current." -{$w} days"));
            if ($t['end'] == "") $t['end'] = $current_end;
           // if ($t['end'] < $current_end) $t['end'] = $current_end;
        }
        ...
        else
        {
            ...
            $this->layout_show('calendar/calendar_new.html',$data);
        }
    }
