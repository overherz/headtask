            ...
            if ($_POST['id'])
            {
                if ($access['access']['news'])
                {
                    ...
                    if ($query->execute(array($_POST['name'],$_POST['description'],time(),$_POST['id'])))
                    {
                        ...
                        if ($log) $log->set_logs("project",$id_project,"Изменил новость <a href='/projects/news/show/{$news['id']}'>\"{$news['name']}\"</a>{$log_text}");
            ...
            else
            {
                if ($access['access']['news'])
                {
                    ...
                    if ($query->execute(array($_POST['name'],$_POST['description'],time(),$_SESSION['user']['id_user'],$id_project)))
                    {
                        ...
                        $res['success'] = $last_id;
                        if ($log) $log->set_logs("project",$id_project,"Добавил новость <a href='/projects/news/show/{$last_id}'>\"{$_POST['name']}\"</a>");
                    }
    ...
    function delete_news()
    {
        ...
        if ($access['access']['news'])
        {
            ...
            if ($query->execute(array($_POST['id'])))
            {
                ...
                $log->set_logs("project",$access['project']['id'],"Удалена новость \"{$news['name']}\"");
            }
