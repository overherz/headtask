            ...
            if ($_POST['id'])
            {
                if ($access['access']['news'] && $access['access']['show_news'])
                {
                    ...
                    if ($query->execute(array($_POST['name'],$_POST['description'],time(),$_POST['id'])))
                    {
                        ...
                        if ($log) $log->set_logs("news",$id_project,"<a href='/projects/news/show/{$news['id']}'>{$news['name']}</a>{$log_text}","edit");
            ...
            else
            {
                if ($access['access']['news'] && $access['access']['show_news'])
                {
                    ...
                    if ($query->execute(array($_POST['name'],$_POST['description'],time(),$_SESSION['user']['id_user'],$id_project)))
                    {
                        ...
                        $res['success'] = $last_id;
                        if ($log) $log->set_logs("news",$id_project,"<a href='/projects/news/show/{$last_id}'>{$_POST['name']}</a>","add");
                    }
    ...
    function delete_news()
    {
        ...
        if ($access['access']['news'] && $access['access']['show_news'])
        {
            ...
            if ($query->execute(array($_POST['id'])))
            {
                ...
                $log->set_logs("news",$access['project']['id'],"{$news['name']}","delete");
            }
