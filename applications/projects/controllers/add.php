        ...
        if (!$res['error'])
        {
            ...
            else
            {
                if ($access['access']['add_project'] || $access['access']['add_own_project'])
                {
                    ...
                    if ($query->execute(array($_POST['name'],$_POST['description'],$_POST['url'],$_POST['archive'],$owner,$GLOBALS['globals']['current_company'])))
                    {
                        $last_id = $this->db->lastInsertId();
                        $res['success'] = $last_id;
                        if ($log) $log->set_logs("project",$last_id,"{$_POST['name']}","add");
                    }
