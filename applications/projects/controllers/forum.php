        ...
        if (!$res['error'])
        {
            ...
            if ($_POST['id'])
            {
                ...
                if ($query->execute(array($_POST['name'],$_POST['id'])))
                {
                    ...
                    if ($log) $log->set_logs("forum",$id_project,"Раздел форума <a href='/projects/forum/show/{$forum['id']}'>{$forum['name']}</a>{$log_text}","edit");
                }
            ...
            else
            {
                ...
                if ($query->execute(array($_POST['name'],$id_project)))
                {
                    ...
                    $res['success'] = $project['id'];
                    if ($log) $log->set_logs("forum",$id_project,"Раздел форума <a href='/projects/forum/show/{$last_id}'>{$_POST['name']}</a>","add");
                }
    ...
    function delete_forum()
    {
        ...
        if ($access['access']['forum'] && $access['access']['show_forum'])
        {
            ...
            if ($query->execute(array($_POST['id'])))
            {
                ...
                $log->set_logs("forum",$access['project']['id'],"Раздел форума {$forum['name']}","delete");
            }
            ...
            else
            {
                ...
                if ($query->execute(array($_POST['name'],$forum['id'],$_SESSION['user']['id_user'],time(),$_POST['fixed'],$_POST['closed'])))
                {
                    ...
                    else
                    {
                        ...
                        $log = $this->get_controller("projects","logs");
                        if ($log) $log->set_logs("forum",$id_project,"Тема <a href='/projects/forum/show_topic/{$id_topic}'>{$_POST['name']}</a>","add");
                    }
                }
    ...
    function delete_topic()
    {
        ...
        if ($access['access']['forum'] && $access['access']['show_forum'])
        {
            ...
            if ($query->execute(array($_POST['id'])))
            {
                $res['success'] = $access['project']['id'];
                $log = $this->get_controller("projects","logs");
                if ($log) $log->set_logs("forum",$id_project,"Тема {$topic['name']}","delete");
            }
            else $res['error'] = "Ошибка базы данных";
        }
        else $res['error'] = "У Вас недостаточно прав";
