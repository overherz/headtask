...
class users extends \Admin {
    ...
    function default_show()
    {
        ...
        if (isset($_POST['search']) && $_POST['search'] != '')
        {
            ...
            $sql = "where u.first_name LIKE ".$s." OR u.last_name LIKE ".$s." OR u.email LIKE ".$s;
        }
    ...
    function save()
    {
        ...
        if (!$res['error'])
        {
            ...
            if ($_POST['id'] != "")
            {
                if ($p)
                {
                    $query = $this->db->prepare("UPDATE users set gender=?,email=?,pass=?,salt=?,uniq_key=?,id_group=?,first_name=?,last_name=? where id_user=?");
                    if (!$query->execute(array($_POST['gender'],$_POST['email'],$p['password'],$p['salt'],$p['uniq_key'],$id_group,$_POST['first_name'],$_POST['last_name'],$_POST['id']))) $res['error']  = true;
                    elseif ($log) $log->save_into_log("admin","Пользователи",true,"Отредактирован пользователь \"{$user['fio']}\". Пароль изменен".$message,$_SESSION['admin']['id_user']);
                }
                else
                {
                    $query = $this->db->prepare("UPDATE users set gender=?,email=?,id_group=?,first_name=?,last_name=? where id_user=?");
                    if (!$query->execute(array($_POST['gender'],$_POST['email'],$id_group,$_POST['first_name'],$_POST['last_name'],$_POST['id']))) $res['error']  = true;
                    elseif ($log) $log->save_into_log("admin","Пользователи",true,"Отредактирован пользователь \"{$user['fio']}\"".$message,$_SESSION['admin']['id_user']);
                }
            }
            else
            {
                $query = $this->db->prepare("INSERT INTO users(gender,email,id_group,first_name,last_name,pass,salt,uniq_key,mailconfirm,created) VALUES(?,?,?,?,?,?,?,?,?,?,?)");
                if (!$query->execute(array($_POST['gender'],$_POST['email'],$id_group,$_POST['first_name'],$_POST['last_name'],$p['password'],$p['salt'],$p['uniq_key'],true,time()))) $res['error']  = true;
                elseif ($log) $log->save_into_log("admin","Пользователи",true,"Добавлен пользователь \"{$_POST['fio']}\"".$message,$_SESSION['admin']['id_user']);
            }
