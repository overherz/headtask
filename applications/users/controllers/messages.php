    ...
    {  
        ...
        if (!$this->id)
        {
            ...
            while ($row = $query->fetch()) 
            {
                ...
                $dialogs[] = $row;
                $ids[] = $row['id_user']; $ids[] = $row['to_user'];
            }
            if ($ids)
            {
                $ids = array_unique($ids);
                $ids = implode(",",$ids);
                $query = $this->db->query("select id_user,fio,nickname,avatar,gender from users where id_user IN({$ids})");
        ...
        else 
        {
            ...
            if ($id > 0)
            {
                $query = $this->db->prepare("select m.*,u.fio,u.nickname,u.gender,u.id_user,u.avatar from messages as m
                ...
                $query->execute(array($_SESSION['user']['id_user'],$id,$id,$_SESSION['user']['id_user']));
                $count = $query->fetch();
                $query = $this->db->prepare("SELECT fio,avatar,gender from users where id_user = ? limit 1");
            ...
            $this->layout_show("dialog.html",array('messages' => $messages,'opponent' => $this->id,'count' => $count['count'], 'opavatar' => $opavatar));
        }
    }     
